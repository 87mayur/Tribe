...
function RegistrationPanel({ legalDocs, onSignup, addEvent, activeUser, setActiveUser }) {
  ...
  const generateDummy = () => {
    ...
    setForm(dummy);
    const allAccepted = Object.fromEntries(enabledDocs.map((doc) => [doc.id, true]));
    setAcceptedDocs(allAccepted);
  ...
  return (
    <Card className="h-full rounded-3xl border border-emerald-400/20 bg-emerald-400/5 text-white">
...
function ClientPortalPanel({ activeUser, addEvent }) {
  ...
  return (
    <Card className="h-full rounded-3xl border border-sky-400/20 bg-sky-400/5 text-white">
      <CardContent className="space-y-4 p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
...
function AdminBackendPanel({ users, legalDocs, setLegalDocs, activeUser, addEvent, seedDemo }) {
  ...
  return (
    <Card className="h-full rounded-3xl border border-violet-400/20 bg-violet-400/5 text-white">
      <CardContent className="space-y-4 p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
      ...
      <header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/90 backdrop-blur">
        <div className="mx-auto flex max-w-[1900px] items-center justify-between px-5 py-4">
          ...
          <div className="hidden items-center gap-2 md:flex">
            {[["sandbox", "Sandbox"], ["registration", "Registration"], ["portal", "Client Portal"], ["admin", "Admin"]].map(([key, label]) => (
